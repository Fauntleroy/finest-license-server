(async () => {
  try {
    ...
    if (activeScripts?.shopify) { eval(activeScripts.shopify); return; }
  } catch {}

/**
 * Finest Checkouts — Shopify Checkout Autofill
 */
...
(function () {
  ...
  // Payment selectors — matches by name, id, placeholder, or aria-label
  const PAY = {
  ...
  async function fillContact(profile) {
    ...
    // Country first — Shopify repopulates state after country change
    ...
    // Expiry — fill combined OR split, never both
