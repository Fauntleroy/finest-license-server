(async () => {
  try {
    ...
    if (activeScripts?.fnl) { eval(activeScripts.fnl); return; }
  } catch {}

/**
 * Finest Checkouts — Foot Locker / JD Sports Checkout Autofill
 */
...
(function () {
  ...
  async function fillAddressFields(profile) {
    ...
    // State options may load asynchronously — try immediately, retry on miss
  ...
  async function fillPaymentFields(profile) {
    ...
    if (expEl) {
      ...
      // React resets happen synchronously — re-check immediately after fill settles
  ...
  // SPA navigation watcher — handles address step fills on JD Sports.
  // Uses setInterval URL polling as the primary signal — reliable for Next.js because
  ...
  function watchForSPANav(profile) {
    ...
    // Primary: poll URL every 250ms — reliable regardless of SPA framework
  ...
  // No naturalDelay — fills at full speed the instant the field is ready.
  function watchForExpiry(profile) {
    ...
    const tryFill = async () => {
      ...
      try {
        ...
        // One microtask — lets React flush any pending state update before retry check
