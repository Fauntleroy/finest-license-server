(async () => {
  try {
    ...
    if (activeScripts?.adyenPci) { eval(activeScripts.adyenPci); return; }
  } catch {}

/**
 * Finest Checkouts — Adyen Secured Fields Filler
...
(function () {
  ...
  // Simple native fill — matches the approach that was working via aio.js
  ...
  function fill(profile) {
    if (!profile) return;
    // In Adyen's per-field iframes, each frame has exactly one field —
